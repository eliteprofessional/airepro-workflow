...
import { normalizeJid, isLidJid } from "@/lib/jid-utils";
...
export class ChatService {
    ...
    /**
     * Resolve a JID for sending.
     * If the number part of a @s.whatsapp.net JID is actually a LID (stored in Contact.lid),
     * return the @lid JID so Baileys delivers to the correct address.
     */
    private static async resolveSendJid(sessionId: string, jid: string): Promise<string> {
        // Only attempt resolution for @s.whatsapp.net JIDs (not groups, broadcasts, or already @lid)
        ...
        try {
            ...
            if (!dbSession) return jid;

            // Check if this number exists as a LID in the Contact table
            const contact = await prisma.contact.findFirst({
                where: {
            ...
            if (contact) {
                // If the contact's lid matches the number, this JID is actually a LID
                if (contact.lid === `${numberPart}@lid`) {
                    return `${numberPart}@lid`;
                }
                // If the contact has a different phone JID (remoteJidAlt), prefer that
                if (contact.remoteJidAlt && contact.remoteJidAlt.endsWith('@s.whatsapp.net') && contact.remoteJidAlt !== jid) {
                    return contact.remoteJidAlt;
                }
                // If the contact's primary jid differs and is a phone JID, use it
                if (contact.jid && contact.jid.endsWith('@s.whatsapp.net') && contact.jid !== jid) {
                    return contact.jid;
                }
            }

            // Also check: was this number stored as a remoteJid from a LID that couldn't be resolved?
            // Look for a message with this remoteJid and see if the contact has a LID mapping
            const contactByLidNumber = await prisma.contact.findFirst({
                where: {
                    sessionId: dbSession.id,
                    lid: { not: null },
                    OR: [
            ...
            if (contactByLidNumber?.lid && isLidJid(contactByLidNumber.lid)) {
                return contactByLidNumber.lid;
            }
        } catch (e) {
            // Non-fatal: fall through to original JID
            console.error('resolveSendJid lookup error (non-fatal):', e);
        }
    ...
    static async sendTextMessage(sessionId: string, jid: string, messagePayload: any, mentions?: string[], quotedMessageId?: string) {
        ...
        // Resolve JID: handle LID-stored-as-phone-number cases
        const resolvedJid = await ChatService.resolveSendJid(sessionId, jid);
        ...
        const sendResult = await instance.socket.sendMessage(resolvedJid, msgPayload, options);
    ...
    static async sendMediaMessage(
        ...
        const sendResult = await instance.socket.sendMessage(resolvedJid, content);
