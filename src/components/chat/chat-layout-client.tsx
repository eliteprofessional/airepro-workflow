    ...
    useEffect(() => {
        const handlePopState = async () => {
            ...
            if (path.startsWith("/dashboard/chat/")) {
                ...
                // Try to find correct JID format from existing messages via API
                try {
                    const res = await fetch(`/api/chat/${sessionId}/resolve-jid?number=${clean}`);
                    if (res.ok) {
                        const data = await res.json();
                        if (data.jid) {
                            setSelectedChat({ jid: data.jid });
                            return;
                        }
                    }
                } catch {
